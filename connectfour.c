	...
	do /*Bei korrekter Eingabe wird überprüft ob man noch in die gewählte Spalte einwerfen kann*/
	{
		do /*Eingabeüberprüfung*/
		{
			if (scanf("%d", &eingabe) > 0)/*Scanf gibt den Wert 0 bei fehlerhafter Eingabe zurück*/
			{
				...
				{
					printf("\nDo you really want to cancel the game? (y/n)\n");
					do
					{
						if (scanf("%c", &c)>0)
						{
				...
				{
					printf("\nPlease insert a number between 1 and 7!\n");
					eingabe=8;
				}
			}
			else
...
int Spiel (int Spiel) /*Funktion zum Aufrufen der einzelnen Spielfunktionen*/
{
	...
	int i=0,j=0,g='a';
	printf("\nPlayer 1 please enter your name (max 20 characters):\n");
	scanf("%s",name);
	if (Spiel==2)/*Wird im Hotseatmodus gespielt so wird ein zweiter Spielername abgefragt*/
	{
		printf("\nPlayer 2 please enter your name (max 20 characters):\n");
		scanf("%s",name2);
	}
	printf("\n  1  2  3  4  5  6  7");
	...
	while (g==0)/*Solange kein Gewinner feststeht ist g=0*/
	{
		...
		{
			printf("\nThe magic machine will now determine who will be the first to play....\n\n");
			i=rand()%2;
		}
		if (i==1)
			g=Eingabe(name, i);
...
int main (void)
{
	...
	do
	{
		...
		printf("\n1: New Game against the PC\n\n2: New Game: Player vs. Player (Hotseat)\n\n3: Rules\n\n4: Exit Game\n\n");
		textcolor(RESET, WHITE, BLACK);
		if (scanf("%d",&menu)>0)
		{
			if (menu==1)
			...
			if (menu==2)
				Spiel(2);
			if (menu==3)
				printf("\nTry to connect four stones in a vertical, horizontal or diagonal row as fast as possible.\nThe first one to achieve this wins the game!\n");
			if (menu==5)
				debug*=-1;
		}
		else
